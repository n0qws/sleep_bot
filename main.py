...
async def handle_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "log_sleep":
        ...
        await callback.message.answer("🛌 Сколько часов ты спал? (например: 7.5)")
        await callback.answer()
    
    elif action == "stats":
        data = get_last_7_days(user_id)
        if not data:
            await callback.message.answer("📭 Нет данных за 7 дней. Запиши свой сон!")
        else:
        ...
        await callback.message.answer("⏰ Во сколько ты просыпаешься? (ЧЧ:ММ, например 07:30)")
        await callback.answer()
...
async def process_sleep(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("⭐ Оцени качество сна от 1 до 10 (10 — отличный сон)")
    except ValueError:
...
async def process_quality(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("😊 Оцени настроение сегодня от 1 до 10")
    except ValueError:
...
async def process_mood(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("🕐 Во сколько ты лёг спать? (ЧЧ:ММ, например 23:30)")
    except ValueError:
        await message.answer("❌ Введи целое число от 1 до 10")
...
async def calculate_time(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("❌ Неправильный формат. Введи время как 07:30")
